...
//         const Condition& rCondition = KratosComponents<Condition>::Get("Condition3D9N"); 
        ...
        )
    {
        ...
        {
            if (EdgeGeometry[i_node].IsDefined(INTERFACE) == true)  
            {
                if (EdgeGeometry[i_node].Is(INTERFACE) == true) ++count;
            }
        }
        ...
        if (count == number_of_points)
        {
            ...
            if (number_of_points == 2)
            {
                ...
                CreateNewCondition(rInterfacePart, rpElem, EdgeGeometry, CondId, EdgeConditionName);
                ++CondCounter;
            }
            else
            {                            
                ...
                else
                {
                    ...
                    CreateNewCondition(rInterfacePart, rpElem, lin_1, CondId, EdgeConditionName);
                    ++CondCounter;
                    ++CondId;
                    Line2D2< Node<3> > lin_2(EdgeGeometry(1), EdgeGeometry(2));
                    CreateNewCondition(rInterfacePart, rpElem, lin_2, CondId, EdgeConditionName);
                    ++CondCounter;
                }
            }
        }
